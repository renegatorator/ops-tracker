...
- `assignIssue` and `softDeleteIssue` revalidate the affected issue detail path; status CRUD revalidates issues list and admin statuses; `getLocalizedSeoMetadata` recognizes `/admin`, `/admin/users`, `/admin/statuses`, `/admin/settings`
